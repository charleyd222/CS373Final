...
def test_model(model, start_sequence, max_length):
    ...
    for _ in range(max_length):
        ...
        predicted_event = predicted_event.item() # get data from tensor
        generated_sequence.append(predicted_event) # add predicted event to sequence
        input_seq.cat(torch.tensor(predicted_event)) # add predicted event to next input
...
if __name__ == "__main__":
    ...
    model = rnn_model(input_size, hidden_size, output_size, 2)
